...
class Domino {
        ...
        getVariableSquare(gGSP) {
                let getGridSubPos = gGSP;
                if (!getGridSubPos) {
                        getGridSubPos = SignTableau.getGridSubPos;
                }
...
class Tableau {
        ...
        flipDominoForCycle(domino) {
                ...
                let x = domino.x;
                let y = domino.y;
                let fs = domino.getFixedSquare(this.getGridSubPos);
                if (domino.horiz) {
                        domino.horiz = false;
                        if (fs.x == x) {
                                // grid[y][x + 1] = undefined;
        ...
        getForwardSquare(backSquare) {
                ...
                let boxed = this.getGridSubPos(square) == "W";
                let domino = grid.get(square.x, square.y);
                do {
                        let fixedSquare = domino.getFixedSquare(this.getGridSubPos);
                        let x = fixedSquare.x;
                        let y = fixedSquare.y;
                        let gridPos = this.getGridSubPos(fixedSquare);
        ...
        getBackSquare(forwardSquare) {
                ...
                let boxed = this.getGridSubPos(square) == "X";
                let domino;
                do {
                        let x = square.x;
                        let y = square.y;
                        let gridPos = this.getGridSubPos(square);
                        if ((boxed && gridPos == "X") || (!boxed && gridPos == "W" ) ) {
                                ...
                                if (!choice1 && !choice2) {
                                        ...
                                        domino = choice1;
                                        square = domino.getVariableSquare(this.getGridSubPos);
                                } else {
                                        domino = choice2;
                                        square = domino.getVariableSquare(this.getGridSubPos);
                                }
...
class TableauB extends Tableau {
        constructor(dominoList) {
                if (!dominoList || dominoList.length == 0) {
                        let dominoList0 = [new Domino({n: 0, x: 0, y: 0, zero: true})];
                        super(dominoList0);
                } else {
                        super(dominoList);
                }
                ...
                this.getGridSubPos = function(square) {
        ...
        clone() {
                let newDominoList = [];
                this.dominoList.forEach((d) => { newDominoList.push(d.clone()); });
                return new TableauB(newDominoList);
        }

}

class TableauC extends Tableau {
        constructor(dominoList) {
                super(dominoList);
                this.offset = {x: 0, y: 0};
                this.shift = function(obj) {
        ...
        clone() {
                let newDominoList = [];
                this.dominoList.forEach((d) => { newDominoList.push(d.clone()); });
                return new TableauC(newDominoList);
        }

}

class TableauD extends Tableau {
        constructor(dominoList) {
                super(dominoList);
                this.offset = {x: 1, y: 0};
                this.getGridSubPos = function(square) {
        ...
        clone() {
                let newDominoList = [];
                this.dominoList.forEach((d) => { newDominoList.push(d.clone()); });
                return new TableauD(newDominoList);
        }

}

class SignTableau{
        constructor(tableau, pairedType) {
                if (!tableau) {
                        this.tableau = new TableauD([]);
                } else {
        ...
        clone() {
                ...
                return new SignTableau(tab, this.pairedType);
        }
        ...
        hasPairFor(domino) {
                ...
                if (SignTableau.getGridSubPos(fs) != "Y") {
        ...
        isCycleTop(domino) {
                ...
                if (SignTableau.getGridSubPos(domino) != "X" || this.hasPairFor(domino)) {
        ...
        isCycleBottom(domino) {
                ...
                if (SignTableau.getGridSubPos(fixedSquare) != "Y" || this.hasPairFor(domino)) {
                        throw new Error("Bad input to isCycleTop");
                }
        ...
        isTopCorner(domino) {
                if (SignTableau.getGridSubPos(domino) != "Y") {
        ...
        moveCycleUpFromSquare(square, domino) {
                ...
                if (!domino) {
                        ...
                        // this.tableau.dominoGrid.grid[y][x] = undefined;
                } else {
        ...
        premakeBox(pos) {
                ...
                let gridPos = SignTableau.getGridSubPos(pos);
                if (gridPos == "X" && !horiz) {
                        this.putBox(x - 1, y);
...
class DualSignTableaux {
        constructor(tbl) {
                if (tbl) {
                        ...
                        this.stab = new SignTableau(false, "B");
                        this.dstab = new SignTableau(false, "C");
                }
        }
...
class DualTableaux {
        constructor(tbl) {
                if (tbl) {
                        ...
                        this.tab = new TableauB([]);
                        this.dtab = new TableauC([]);
                }
        }
...
class Tableaux {
        ...
        draw() {
                let tabx = this.clone();
                let stab = tabx.dualSignTabs.stab;
                let row0Length = stab.getRowLength(0);
                let row1Length = stab.getRowLength(1);
                for (let x = row1Length + 2; x < row0Length; x += 2) {
                        stab.get(x, 0).invisible = true;
                }

                let dstab = tabx.dualSignTabs.dstab;
                for (let y = row1Length + 3; y < row0Length + 1; y += 2) {
                        dstab.get(0, y).invisible = true;
                }

                let tabPair1 = new TableauPair(tabx.dualTabs.tab, tabx.dualSignTabs.stab.tableau);
                let tabPair2 = new TableauPair(tabx.dualTabs.dtab, tabx.dualSignTabs.dstab.tableau);
        ...
        putSignInCycleTop(topDomino1, sign1, sign2, noTab) {
                ...
                if (adjacentSign1 == "" && bottomSign1 == "" && boxed) {
                        ...
                        if (!noTab) {
                                let square1 = {x: bfs1.x + 1, y: bfs1.y};
                                stab1.shift(square1);
                                tab1.moveOpenCycle(square1);
                                let square2 = {x: bfs1.y, y: bfs1.x + 1};
                                stab2.shift(square2);
                                tab2.moveOpenCycle(square2);
        ...
        putSignInCycleBottom(bottomDomino1, sign1, sign2, noTab) {
                ...
                if (aboveSign1 == "" && topSign1 == expectedTopSign1 && !boxed) {
                        ...
                        if (!noTab) {
                                let square1 = {x: tfs1.x, y: tfs1.y + 1};
                                stab1.shift(square1);
                                tab1.moveOpenCycleToSquare(square1);
                                let square2 = {x: tfs1.y + 1, y: tfs1.x};
                                stab2.shift(square2);
                                tab2.moveOpenCycleToSquare(square2);
        ...
        addNumberSign(number, sign1, y) {
                ...
                let gridPos = SignTableau.getGridSubPos({x: x, y: y});
                ...
                if (gridPos == "X") {
                        ...
                        if (cond1 && cond2) { // both filled, make new up cycle
                                ...
                                domino1n.n = number;
                                domino2n.n = number;
                                stab1.shift(domino1n);
                                stab2.shift(domino2n);
                ...
                domino.n = number;
                dualDomino.n = number;
                this.dualSignTabs.stab.shift(domino);
                this.dualSignTabs.dstab.shift(dualDomino);
        ...
        openTwoCycles(startDomino1, targetDomino1, stab1, stab2, tab1, tab2) {
                ...
                if (!targetDomino1.isBoxed()) {
                        let square1 = {x: targetDomino1.x, y: targetDomino1.y + 1};
                        stab1.shift(square1);
                        tab1.moveOpenCycle(square1);
                } else {
                        let square2 = {x: targetDomino1.y + 1, y: targetDomino1.x};
                        stab2.shift(square2);
                        tab2.moveOpenCycleToSquare(square2);
                }
        ...
        static SOpqSpnRRobS(parameter) {
                let myTableaux = new Tableaux();
                let firstSign = parameter[0];
                let opFirstSign = oppositeSign(firstSign);
                let stab = myTableaux.dualSignTabs.stab;
                let dstab = myTableaux.dualSignTabs.dstab;
                stab.putDomino({n: firstSign, x: 0, y: 0, horiz: false});
                dstab.putDomino({n: "", x: 0, y: 0, horiz: false});
                for (let number = 0, end = (parameter.length - 1) / 2; number < end; ++number) {
                        stab.putDomino({n: opFirstSign, x: 4 * number + 1, y: 0, horiz: true});
                        stab.putDomino({n: firstSign, x: 4 * number + 3, y: 0, horiz: true});
                }

                let end = (parameter.length % 2 == 0)? parameter.length + 1: parameter.length
                for (let number = 1; number < end; ++number) {
                        dstab.putDomino({n: "", x: 0, y: 2 * number, horiz: false});
                }

                for (let number = 1; number < parameter.length; ++number) {
                        let info = parameter[number];
                        if (info == "+" || info == "-") {
                                myTableaux.addNumberSign(number, info, 1);
                        } else if (info == "s" || info == "t") {
                                ...
                                let gridPos = SignTableau.getGridSubPos(pos);
                                let dGridPos = SignTableau.getGridSubPos(dpos);
                                ...
                                if ((gridPos == "Y" && pos.horiz) || (dGridPos == "Y" && dpos.horiz)) {
                                        ...
                                        myTableaux.dualSignTabs.stab.shift(domino);
                                        myTableaux.dualSignTabs.dstab.shift(dualDomino);
                                        ...
                                        let x = pos1.x;
                                        let y = pos1.y;
                                        let squareToForward = {x: x + 1, y: y};
                                        stab1.shift(squareToForward);
                                        let forwardSquare = tab1.getForwardSquare(squareToForward);
                                        tab1.shift(forwardSquare);
                                        let squareToBack = {x: x, y: y + 1};
                                        stab1.shift(squareToBack);
                                        let backSquare = tab1.getBackSquare(squareToBack);
                                        tab1.shift(backSquare);
                                                        ...
                                                        if (cornerDomino2.n != sign2) {
                                                                ...
                                                                if (rowAddSign2 > x + 1) {
                                                                        ...
                                                                        if (open) {
                                                                                ...
                                                                                let square2 = {x: y, y: x + 1};
                                                                                stab2.shift(square2);
                                                                                if (tab1.dominoGrid.get(square1.x, square1.y + 1)) {
                                                                                        domino1 = new Domino({n: "", x: x + 1, y: y, horiz: false});
                                                                                        domino2 = new Domino({n: "", x: y, y: x + 1, horiz: true});
                                                                                        tab1.moveOpenCycle({x: square1.x + 1, y: square1.y});
                                                                                        tab2.moveOpenCycle({x: square2.x, y: square2.y});
                                                                                }
        ...
        static SOpqSpnRRobSDraw(parameter) {
                let myTableaux = new Tableaux();
                let firstSign = parameter[0];
                let opFirstSign = oppositeSign(firstSign);
                let stab = myTableaux.dualSignTabs.stab;
                ...
                for (let number = 1; number < parameter.length; ++number) {
                        ...
                        if (info == "+" || info == "-") {
                                ...
                                posDomino = new Domino(posDomino);
                                myTableaux.dualSignTabs.stab.shift(posDomino);
                                posDomino.highlight = 5;
                                dposDomino = new Domino(dposDomino);
                                myTableaux.dualSignTabs.dstab.shift(dposDomino);
                                dposDomino.highlight = 5;
        ...
        static SOpqSpnRRobSString(paramString) {
                let parameter = Parameter.parseParameterString(paramString);
                return Tableaux.SOpqSpnRobS(parameter);
        }
...
class Parameter {
        ...
        parseParameter() {
                ...
                let output = "";
                for (let number = 0; number < parameter.length; number++) {
                        if (parameter[number] == "+") {
                                output += number + "+ ";
...
class TableauRendererDOM {
        constructor(tbl) {
                ...
                this.offset = this.tableau.offset || tbl.offset || {x: 0, y:0}
        }
        ...
        renderDOM() {
                ...
                for (let d of this.tableau.dominoList) {
                        ...
                        if (d.zero) {
                                dominoElem.className += " dominoZero";
                        } else if (d.box) {
